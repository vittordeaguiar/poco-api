...
export const AppLayout = () => {
  ...
  return (
    <div className="app-shell relative flex min-h-screen flex-col overflow-x-hidden">
      <header className="glass-bar sticky top-0 z-20 flex items-center justify-between border-b border-border bg-[var(--topbar-bg)] px-6 pb-4 pt-5">
        <div>
      ...
      <main className="relative z-10 flex-1 px-6 pb-24 pt-5">
        <Outlet />
      </main>

      <nav className="glass-bar scrollbar-hide sticky bottom-0 z-20 mx-4 mb-4 grid grid-flow-col auto-cols-fr gap-1.5 overflow-x-auto rounded-2xl bg-[var(--bottombar-bg)] px-3 py-2">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
